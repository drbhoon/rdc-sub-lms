...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  details.addRow(["Employee Code", "Learner", "Email", "Company", "Location", "Feedback form", "Version", "Date", "Time", ...Array.from({ length: maxQuestions }, (_, index) => `Q${index + 1}`), "Submission"]);
  ...
  for (const form of course.feedbackForms) {
    for (const response of form.responses) {
      ...
      details.addRow([
        ...
        response.employee.locationPlant ?? "", form.title, form.version, response.submittedAt, response.submittedAt,
  ...
  details.getColumn(10 + maxQuestions).numFmt = "yyyy-mm-dd hh:mm:ss";
  autoFit(details);
  for (let column = 10; column < 10 + maxQuestions; column += 1) {
